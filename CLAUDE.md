...
uv run scripts/gestionar_web.py edit-documento [<slug>]  # transcribir/editar un documento curado (content/documentos/*.md)
...
- `index.html` — **home "Índice + log"**: hero + **Áreas** (tres tarjetas de color sólido, `.cmzo-gcard` — blog · lab · genealogía) + **log** (bitácora que fusiona `blog-entries.json` + `lab-entries.json` + `notas.json` + `changelog-entries.json` por fecha; `post`/`lab` navegan a su página, `nota`/`imagen`/`enlace`/`cambio` abren modal; `imagen` usa el lightbox compartido `assets/js/lightbox.js`). Límite 10 + "ver más". Solo-ES (sin i18n). Expone `window.__openNota(id)` y lee `?nota=<id>`. Los títulos de sección (`.cmzo-mod-head h2`) van **sin numerar** (el «01»/«02» se sacó 2026-08-14) y comparten tamaño (48px, versalitas) con los de `/gen`, `/fuentes`, `/colaborar` y el `<h1>` de `/lab`.
...
- `gen.html` — **índice del área genealogía**: hero + una grilla de **cinco tarjetas de color sólido** (`.cmzo-gcard`): árbol · wiki · fuentes · hipótesis · colaborar.

**`.cmzo-gcard` (`home.css`)** — componente de tarjeta general del sitio, rediseño 2026-08 desde una referencia de Uiverse; grilla `.cmzo-gcards` de **3 por fila**. Lo usan `/gen` (5 tarjetas: árbol/wiki/fuentes/hipótesis/colaborar), la home (Áreas: blog/lab/genealogía) y `/lab` (los tres laboratorios) — reemplazó tanto a `.cmzo-area` (portada + pastilla) como a `.lab-card` y a las cuatro secciones a dos columnas con paneles `.cmzo-viz`/`.cmzo-solid` de `/gen` (con ellas se retiraron `.cmzo-rings`, `.cmzo-solid`, `.cmzo-meter`, `.cmzo-two`, `.cmzo-area*` y `.lab-card*`). **La tarjeta entera es el pastel** — sin borde, sin portada/imagen. Racimo de círculos superpuesto solo donde hay **individuos reales que nombrar** (iniciales de la línea directa en el árbol, hubs del grafo en la wiki); si no hay individuos, el badge va solo (Fuentes, Hipótesis, Colaborar, y las tres del lab). Colores por familia (`--pill-escritura/--experim/--archivo/--hipotesis`, e `--investig` por defecto — mismos tokens que `.cmzo-pill`, **fijos en ambos temas**: son objetos de papel). **Indicador + barra** al cierre; la barra **solo aparece donde hay una proporción real** que mostrar (blog: % de posts en francés; árbol: % con fecha de nacimiento; fuentes: % suizas; wiki: % de nodos documentados; hipótesis: % de hipótesis sólidas/confirmadas — lab y Colaborar no llevan porque no hay nada real que medir).
- `hipotesis.html` — **hipótesis abiertas + tareas pendientes, centralizadas**. Tres secciones: **Llaves** (curadas a mano — hipótesis marcadas con `llave_*` en su frontmatter porque su tarea pendiente destraba varias cosas a la vez, ordenadas con la más reciente arriba vía badge "NUEVO"), **Por hacer** (los `- [ ]` de `content/personas/*.md`, autogenerados y clasificados en tabs por tipo de trámite — nacimiento/matrimonio/defunción/censos/archivo/otros — heurística por palabras clave en `build-hipotesis.js`, no siempre perfecta) y **Hipótesis** (cards curadas de `content/hipotesis/*.md`, cross-cutting — no atadas a una sola persona, ej. el patrón uxorilocal). **Estado como pastilla** (`.hyp-status`, `assets/css/hipotesis.css`): misma receta que `.tag-pill` de la wiki (texto de color sobre fondo mezclado 19% con la superficie), con la escala cálida propia `--stat-terracota/--stat-ochre/--stat-salvia` (`styles.css`, junto a `--tag-*`) para especulativa/plausible/sólida — confirmada usa `--accent` directo. Reemplazó una primera versión con punto + azul/naranja/verde genérico (2026-08-13, sin relación con la paleta del sitio) tras iterar con el usuario. **Tabs de "Por hacer" = `.cmzo-fpill`/`.cmzo-filterbar`** (`home.css`), el mismo chip que el filtro del log de la home — no un componente de tabs propio. **Popup estilo Gwern**: cualquier `[[slug|alias]]` dentro de una nota de persona o de una tarea del ToDo se resuelve a un `<span class="hyp-ref" data-hyp="slug">`; al hacer hover (JS inline de la página) aparece una tarjeta flotante con la hipótesis completa (misma pastilla `.hyp-status`), sin navegar. Personas con dudas abiertas en su propia nota (`[!duda]`, `assets/data/dudas.json`) suman además un hover automático (`.has-dudas`, subrayado punteado) sobre su tag — no requiere `[[link]]` explícito, es por estar en la misma nota; popup propio (`.hp-status.dudas`, pastilla neutra) con hasta 3 preguntas + "+N más en su ficha". `build-hipotesis.js` (llamado por `build.js` después de `build-wiki.js`) emite `assets/data/hipotesis.json`, `assets/data/dudas.json` y `assets/data/todo.json`; corre después de `buildArbolData()` porque necesita los nombres de `arbol.json`.
...
Self-contained command palette (Spotlight/Raycast style) opened with **⌘/Ctrl + K**. Injects its own CSS + DOM and a "Buscar ⌘ + K" trigger button into `.nav-actions`. Busca **y ejecuta**: indexa **pages**, **personas** (`arbol.json`), **blog posts** (`blog-entries.json`), **notas** (`notas.json`, busca su contenido; al elegir una abre el modal vía `window.__openNota` o navega a `index.html?nota=<id>`), **páginas de la wiki** (`wiki-graph.json`, grupo **«Wiki»**: lugares/fuentes/eventos/temas con contenido; al elegir una **abre la lectura** vía `window.__wikiRead(id)` si estás en la wiki, o navega a `wiki.html?read=<id>`), **fuentes** (`fuentes.json`, grupo «Fuentes»: Enter = `window.open(url, '_blank', 'noopener')` si hay url, si no navega a `fuentes.html#f-<id>`) y **documentos** (`documentos.json`, grupo «Documentos»: busca por título, personas asociadas y el texto de la transcripción; Enter abre la **vista enfocada** del documento vía `window.__wikiOpenDoc(slug)` si estás en la wiki, o navega a `wiki.html?doc=<slug>`). Suma también un grupo **«Recientes»** (aparece al abrir sin query: últimas 5 selecciones en `localStorage["cmdk-recent"]`, tipos `persona/page/post/wiki/nota/fuente/documento`, icono reloj) y un grupo **«Comandos»** con 2 acciones: alternar tema día/noche + copiar enlace de la página actual. Datos fetched lazily and cached. Búsqueda **multi-palabra** (tokens AND; títulos con coincidencia difusa, resto del haystack solo substring), insensible a acentos; **personas indexadas también por lugares y años** de nacimiento/defunción («riddes 1858» → François); **grupos ordenados inter-sí por relevancia** (score del mejor resultado de cada grupo); keyboard nav; el **resaltado marca cada token tanto en el título como en el subtítulo**. Elegir una persona **prioriza abrir su investigación** (buscar un nombre casi siempre significa querer leerla, no solo ubicarla): si `it.read` es true, Enter llama a `window.__wikiRead(id)` sin salir si ya estás en la wiki, o navega a `wiki.html?read=<id>`; si no tiene investigación, cae al foco in-place vía `window.__personaFocus || window.__treeFocus` (el árbol expone `__treeFocus`; la wiki expone `__personaFocus`), o si la página no define handler, navega a `wiki.html?focus=<id>` (2026-08-14; antes era siempre foco, primero en árbol y después en wiki). En desktop (ancho > 960px) las filas de **persona y wiki** tienen **acción secundaria con `→` y `›`**: para persona → línea de tiempo (`window.__openTimeline` o `arbol.html?timeline=<id>`); para wiki → «Ver en el grafo» (foco en grafo). El footer muestra el hint solo cuando el ítem activo es persona o wiki. En mobile la secundaria se inhibe. **Accesibilidad**: `role="combobox"`, `aria-expanded`, `aria-controls="cmdkList"`, `aria-selected` en ítems, `aria-activedescendant` actualizado en cada movimiento. En la wiki, el filtro por familia es un **sub-menú**: una única entrada «Filtrar por familia…» (tipo `mode`) que al activarse cambia el placeholder a «Tipeá un apellido…» y lista solo las familias; Esc o Backspace con el input vacío vuelven a la raíz; si hay filtro activo aparece además «Quitar filtro de familia». Incluido en Inicio, Árbol, Wiki, Blog, Cambios y posts del blog — **no** en Colaborar ni Fuentes. Paths are relative; `ROOT` is `../../` inside `dist/blog/` posts, `''` elsewhere.
...
1. Lee `content/wiki/*.md` (páginas autoradas, con frontmatter `title`/`type`/`summary`/`tags` y enlaces `[[destino]]`/`[[destino|alias]]`) + `content/personas/p{id}.md` (las **notas de investigación**; las que aún contienen el marcador `_Investigación pendiente._` cuentan como **stub**) + `content/documentos/<slug>.md` (**documentos curados**, ver más abajo) + `arbol.json` (personas + backbone familiar). Una persona tiene página y `hasContent:true` si tiene **nota real o algún documento curado que la nombre**: un stub con documentos genera página con aviso `.wiki-pending` + galería; un stub sin documentos queda como nodo sin botón «Leer».
2. Resuelve enlaces: en páginas, `[[p26]]` / `[[slug]]` / `[[Título]]`; en notas de persona, las **menciones en prosa tipo `p36`** generan aristas; cada **post** se enlaza a las personas y/o páginas de su frontmatter `wiki: "p36, metodologia, …"`; los `tags:` (de páginas, notas y posts) generan aristas `rel:"tag"`. Las notas de persona **también** aceptan `[[destino|alias]]` explícito (además de las menciones bare `pNN`) — `renderBracketLinks()` en `build-wiki.js` es compartida entre páginas, notas y documentos. `[[slug]]` puede apuntar a una **hipótesis** (`content/hipotesis/*.md`) — resuelve a `../../hipotesis.html#slug` vía un mapa aparte (`hipotesisBySlug`) que **no** pasa por `resolvePageLink()`, para no generar aristas colgantes hacia un nodo que no existe en el grafo (las hipótesis no son nodos de la wiki). **Bug conocido, no resuelto**: el caso persona de `resolvePageLink()` arma el href con `n.url` (pensado para el grafo/SPA, `dist/wiki/pNN.html`) en vez de un relativo (`pNN.html`) como el caso página — un `[[pNN]]` dentro del *cuerpo* de una página de la wiki funciona bien en el modal (el click se intercepta por `data-node`) pero 404ea si se entra directo a `dist/wiki/<slug>.html` y se hace click.
...
4. Emite **`assets/data/documentos.json`**: un registro por documento curado con su HTML ya renderizado (`bodyHtml`), para que la wiki lo inyecte directo en el modal sin pedir nada más.

**Documentos curados (`content/documentos/<slug>.md`, 2026-08-13):** cartas, censos, actas — un archivo por documento **real**, no por fila de media. Frontmatter `title` + `personas: pNN, pNN` (a quiénes atañe, sin "pertenecer" a ninguna) + `paginas: archivo1.webp, archivo2.webp` (sus imágenes, en `assets/images/personas/`, en orden). El cuerpo es la transcripción (markdown, soporta `[[links]]`). No genera nodo propio en el grafo — mismo criterio que los tags. La galería «Documentos» al final de la página de cada persona (`mediaHtml()`) muestra cards (miniatura de la primera página + título) de los documentos curados que la nombran; clic abre la **vista enfocada** (`data-doc="<slug>"`), no el artículo completo de la persona. El inventario inicial (176 filas de `media` en `arbol.db` → 99 documentos) se armó agrupando por archivo de imagen (misma url = misma página) y fusionando en un documento las páginas que comparten un `group_label` específico (no genérico — "Censos"/"Fotos"/"Actas" son categorías, no documentos, y quedaron sin fusionar). Editar/transcribir: `uv run scripts/gestionar_web.py edit-documento`.

**Front-end (`assets/js/wiki/graph.js`, ES module; importa `cytoscape` **vendoreado** desde `assets/js/vendor/cytoscape.esm.min.js` — sin CDN; estilos en `assets/css/wiki.css`):** render con **Cytoscape.js** y una **física propia** estilo Obsidian — repulsión entre todos los nodos + centrado + resortes de enlace, **confinados en un borde circular** → el disco redondo emerge de la física (no de post-procesos). El armado son ~300 pasos síncronos con el lienzo oculto (fade-in al terminar). La **misma simulación maneja el arrastre**: agarrar un nodo arrastra a sus vecinos; al soltar todo vuelve al círculo. Constantes ajustables al inicio del bloque de física (`K_REPEL`, `K_CENTER`, `K_LINK`, `K_BOUND`, `DAMP`, `DISC_R`). Color por rama (`getBranchColor` de `arbol/config.js`) o por tipo (`TYPE_COLOR`: familia apagada/cálida, documentada en `design-system.html` §06 y duplicada en los badges/chips de `wiki.css` — cambiar siempre en ambos); los nodos **con contenido legible** (nota/página/post) llevan un **anillo `--accent`**; las aristas `rel:"familia"` van más tenues (opacidad 0.18 vs 0.4) que las de menciones/enlaces; la hoja de estilos se re-aplica al alternar día/noche (MutationObserver sobre `data-theme` → tema vivo); los **hubs** (grado ≥ `HUB_DEG`) muestran el nombre siempre, **todos** al acercar el zoom (> `ZOOM_LABELS`) y los **vecinos** al hover; nombres con **tamaño fijo en pantalla** (se recalcula `font-size` inverso al zoom); hover ilumina las aristas del nodo. El **laboratorio** (`lab-grafo.html`) deja afinar todos estos parámetros en vivo y exporta la receta. Clic → **panel lateral** (`#wikiPanel`) con relaciones + botones "Leer …"/"Ver en árbol"; para una persona, el panel abre con una **ficha fija** (`fichaHtml()`) — nacimiento, defunción (con edad al morir si hay ambas fechas), padres, cónyuge, hermanos, hijos y cantidad de documentos, generada 100% desde `arbol.json` (nunca a mano) e igual en las 133 personas; donde falta un dato queda el hueco a la vista (`—`, o «fecha sin documentar» si hay lugar pero no fecha). El párrafo de resumen se sacó del panel para personas (quedaba duplicado con el del modal); páginas/tags/posts lo conservan, sin ficha. Clicar un link (`data-node`) dentro del **modal** ya no cierra-y-enfoca: si el destino tiene contenido abre su investigación (`openModal`) en el lugar, si no cae a foco simple. El botón **Leer abre un modal** (`#wikiModal`) que muestra **solo el contenido editorial** (texto, imágenes con lightbox, Mermaid) — las relaciones **no se duplican**: el modal se ancla a la izquierda y deja el `#wikiPanel` a la derecha (backdrop uniforme, panel elevado `z-index 1001`; en mobile el modal va a pantalla completa y el panel a drawer). El modal carga `lightbox.js` y Mermaid **vendoreado** (`assets/js/vendor/mermaid.min.js`, build UMD inyectado como `<script>` perezoso — el ESM de mermaid está partido en chunks y no se puede vendorear en un archivo) y re-ejecuta Mermaid tras inyectar el HTML; la página directa hace lo propio en un script inline del template. La tipografía de lectura (`.wiki-page-content`) está documentada en `design-system.html` (sección 06). **Navegación por teclado** (2026-08-16, mismo patrón que `assets/js/arbol/keyboard.js` adaptado a un grafo general): el lienzo (`#wikiGraph`) es focuseable (`tabindex="0"`, outline `--accent` en `:focus-visible`); con foco y sin selección, cualquier flecha o Enter selecciona el hub más conectado del grafo visible como punto de partida; ya con un nodo en foco, →/↓ avanza al vecino visible (no-tag) más conectado, ←/↑ vuelve sobre los pasos (historial de visita tipo "atrás"), Enter/Space abre la lectura si el nodo tiene contenido, Escape suelta el foco. Filtro por familia (vía ⌘K, sub-menú «Filtrar por familia…»), leyenda, zoom. Lee `?focus=<id>` (foco en el grafo), `?read=<id>` (foco + abre el modal de lectura del artículo completo) y `?doc=<slug>` (abre directo la vista enfocada de un documento curado); expone `window.__personaFocus`, `window.__wikiRead` y `window.__wikiOpenDoc`. La **vista enfocada de un documento** (`openDocModal`/`renderDocModal`) arma todo desde `documentos.json` — todas sus páginas + transcripción, con chips de las personas asociadas (clic cierra la vista de documento y abre la ficha completa de esa persona como acción explícita, sin abrir el panel lateral automáticamente).

**Hover-preview estilo Gwern (`assets/js/wiki-hover.js`, 2026-08-14):** autocontenido, sin dependencias del módulo del grafo — solo inyecta un popup (`#wikiHoverPopup`, estilos en `wiki.css`) y escucha `mouseover`/`mouseout` delegados en `document.body`. Escopado a propósito a `.wiki-link[data-node], .wiki-link[data-doc], .wiki-chip[data-node]` (links en prosa y chips de «relacionados») — **no** a `.wiki-doc-card` de la galería «Documentos», que ya es visual (miniatura + título) y no gana nada con una preview redundante encima (decisión explícita del usuario). Al hacer hover: fetch lazy + cacheado de `wiki-graph.json` + `documentos.json` + `arbol.json`; para persona/página muestra badge de tipo, título, rama + fechas de vida si es persona, y el resumen; para documento, título, personas asociadas, miniatura de la primera página y snippet de la transcripción (o «Todavía no tiene transcripción»). **Toda la tarjeta es clickeable** — reenvía el click al elemento original (`el.click()`), así reusa la navegación que cada contexto ya sabe hacer (href real en las páginas directas de `dist/wiki/`, o el delegate de `graph.js` dentro del modal) sin duplicar lógica. `z-index: 1100` (por encima del modal `1000` y del panel `1001` — bug corregido el mismo día, quedaba tapado). Cargado en `wiki.html` y en `content/templates/wiki-template.html` (mismo componente en el SPA y en las páginas directas).

Para agregar contenido: crear `content/wiki/<slug>.md` (lugar/fuente/evento/tema), editar `content/personas/p{id}.md`, crear `content/documentos/<slug>.md` (documento curado), o poner `wiki: "p…, <slug>…"` en el frontmatter de un post; usar `[[…]]`/menciones para enlazar, `![[archivo]]` (en `assets/images/wiki/`) para imágenes y bloques ```mermaid para diagramas, y correr `npm run build`.
