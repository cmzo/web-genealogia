...
Self-contained command palette (Spotlight/Raycast style) opened with **⌘/Ctrl + K**. Injects its own CSS + DOM and a "Buscar ⌘ + K" trigger button into `.nav-actions`. Busca **y ejecuta**: indexa **pages**, **personas** (`arbol.json`), **blog posts** (`blog-entries.json`), **notas** (`notas.json`, busca su contenido; al elegir una abre el modal vía `window.__openNota` o navega a `index.html?nota=<id>`) y **páginas de la wiki** (`wiki-graph.json`, grupo **«Wiki»**: lugares/fuentes/eventos/temas con contenido; al elegir una enfoca el nodo vía `window.__personaFocus` si estás en la wiki, o navega a `wiki.html?focus=<id>`), y suma un grupo **«Comandos»** con acciones (p. ej. alternar tema día/noche, reusando el botón de `theme.js`). Datos fetched lazily and cached. Búsqueda **multi-palabra** (el query se parte en tokens y TODOS deben matchear — AND; el título admite coincidencia difusa, el resto del haystack solo substring), insensible a acentos; las **personas se indexan también por lugares y años** de nacimiento/defunción («riddes 1858» → François); resultados agrupados; keyboard nav; el resaltado marca cada token en el título. Selecting a persona focuses it sin salir vía `window.__personaFocus || window.__treeFocus` (el árbol expone `__treeFocus`; la wiki expone `__personaFocus` para enfocar el nodo en el grafo); si la página no define handler, navega a `arbol.html?focus=<id>`. En desktop (ancho > 960px) las filas de persona tienen **acción secundaria ⌘/Ctrl + Enter → línea de tiempo** (directo vía `window.__openTimeline` —expuesto por `panel.js`— si ya estás en el árbol, o navegando a `arbol.html?timeline=<id>`; el footer muestra el hint solo cuando el ítem activo es una persona). En mobile la secundaria se inhibe y cae a la primaria. En la wiki, el filtro por familia es un **sub-menú**: una única entrada «Filtrar por familia…» (tipo `mode`) que al activarse cambia el placeholder a «Tipeá un apellido…» y lista solo las familias; Esc o Backspace con el input vacío vuelven a la raíz; si hay filtro activo aparece además «Quitar filtro de familia». Included on Inicio, Árbol, Wiki, Blog, Cambios and blog posts — **not** on Colaborar, Fuentes, Sobre. Paths are relative; `ROOT` is `../../` inside `dist/blog/` posts, `''` elsewhere.
...
- `data-page="arbol|wiki|fuentes|colaborar"` — sub-page inside `gen` (renders `cmzo /gen /arbol`). Lab ya no es sub-página de gen: usa `data-section="lab"` + `data-page-label="grafo|fractales"`.
